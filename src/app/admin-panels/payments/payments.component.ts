...
export class PaymentsComponent {
    ...
    search1 = '';
    datatable1Cols = [
        { field: 'paymentId', title: 'PaymentID' },
        { field: 'paymentMethod', title: 'Payment Method' },
        { field: 'planId', title: 'PlanID' },
    ...
    ngOnInit() {
        this.rows = this.rows.map((row) => {
            const status = this.randomStatus(); // Get a random status
            return {
                ...row,
                status: status,
                statusColor: this.getStatusColor(status), // Map status to its specific color
            };
        });
        this.getPaymentData();
    }
    ...
    getStatusColor(status: string) {
        const statusColorMap: { [key: string]: string } = {
            PAID: 'green', // Green for PAID
            PENDING: 'dark-blue', // Dark blue for PENDING
            CANCELED: 'red', // Red for CANCELED
            FAILED: 'yellow', // Yellow for FAILED
            COMPLETE: 'blue', // Blue for COMPLETE
        };

        return statusColorMap[status] || 'gray'; // Default color if status is not matched
    }

    randomStatus() {
        const statuses = ['PAID', 'PENDING', 'CANCELED', 'FAILED', 'COMPLETE'];
        const randomIndex = Math.floor(Math.random() * statuses.length);
        return statuses[randomIndex];
    }

    getPaymentData() {
        this.httpClient.get(this.url + 'api/payments').subscribe(
            (response: any) => {
                // console.log(response);
                this.paymentData = response.payments;
